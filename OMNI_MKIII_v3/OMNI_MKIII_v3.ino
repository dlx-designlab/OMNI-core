...
//******** Network ******************************************************//
int network = 2;  //no netwrork (debug): 0, LTE:1, LoRa:2 
#define CONSOLE_BAUD  9600

//******** select SLEEP Time ******************************************************//
bool bSleep = true;
const unsigned long SLEEP_INTERVAL = 225;// OMNI sleeps for interval * 8sec.
//30min 1800s / 8 = 225
//10min 600s / 8 =  75
//1h 510
const int maxgpstry = 3000; //time for getting GPS signal / 100 sec (9000 = 90sec ))
const int initgpswait = 30; //sec
const int LTEtry = 200; // try LTE connection for 120 times (120 sec)
const int LoRatry = 60;  // try LoRa connection for 60 times (120 sec)

//*****sensor setting******
const int maxECtry = 12;
...
const uint8_t  onBoardTemperature = 11;
...
//******** temperature ******************************************************//
OneWire oneWire1(onBoardTemperature);
OneWire oneWire2(outsideTemperature);
DallasTemperature temperatureSensorsOn(&oneWire1);
DallasTemperature temperatureSensorsOut(&oneWire2);

float temperatureValueOnboard = 0.f;
float temperatureValueOutside = 0.f;
...
void loop()
{
  ...
  if(bSleep){
    wdt_counter++;
   //初回を除き、カウンターがSLEEP_INTERVAL以下なら再度スリープ状態へ
   if(!initFlg && wdt_counter <= SLEEP_INTERVAL ){ 
 ...
 }else if (network ==2){  //senseway lora
  initLoRa();
  if(LoRaJoin()){
...
void getECValue(){
 ...
 // getECserial();
  ...
  float ecv =0;
  float sal =0;
  for(int i =0 ; i < maxECtry ; i++){
    String ECCommand = "RT," + String(temperatureValueOutside); 
...
{
  temperatureSensorsOn.begin();
  temperatureSensorsOn.setWaitForConversion(true);
  temperatureSensorsOut.begin();
  temperatureSensorsOut.setWaitForConversion(true);
}
...
void getTemperature()
{
  ...
  temperatureSensorsOn.requestTemperatures();
  temperatureValueOnboard = temperatureSensorsOn.getTempCByIndex(0);
  temperatureSensorsOut.requestTemperatures();
  temperatureValueOutside = temperatureSensorsOut.getTempCByIndex(0);
